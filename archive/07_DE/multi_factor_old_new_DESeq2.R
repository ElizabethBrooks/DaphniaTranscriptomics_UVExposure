...
design <- read.csv(file="/Users/bamflappy/PfrenderLab/melanica_UV_exposure/old_new_merged/data/study_design.csv", row.names="sample")
...
gene_counts <- dplyr::select(gene_counts, -c("MUV17", "MUV18", "MUV19", "MUV20", "MUV3", "MUV4"))
targets <- targets[!grepl("E2", targets$genotype),]
...
#dds$genotype <- relevel(dds$genotype, ref = "PA13_42")
...
write.csv(as.data.frame(normalized_counts), file="gene_counts_normalized.csv", quote = FALSE, row.names = TRUE)
...
# the effect of treatment in PA (the main effect)
res05 <- results(dds, contrast=c("treatment", "VIS", "UV"), alpha=cutFDR, lfcThreshold=cutLFC)
# the effect of treatment in Sierra
#res05 <- results(dds, contrast=list(c("treatment_UV_vs_VIS", "treatmentUV.groupSierra")), alpha=cutFDR, lfcThreshold=cutLFC)
...
# test if the treatment effect is different in Olympic compared to PA (interaction term)
#res05 <- results(dds, name="treatmentUV.groupOlympic", alpha=cutFDR, lfcThreshold=cutLFC)
# test if the treatment effect is different in Sierra compared to PA (interaction term)
#res05 <- results(dds, name="treatmentUV.groupSierra", alpha=cutFDR, lfcThreshold=cutLFC)
# test if the treatment effect is different in Olympic compared to Sierra (interaction term)
#res05 <- results(dds, contrast=list(c("treatmentUV.groupOlympic", "treatmentUV.groupSierra")), alpha=cutFDR, lfcThreshold=cutLFC)
...
## with treatment, what is the difference between Sierra and Olympic? (double check)
##res05 <- results(dds, contrast=list(c("group_Olympic_vs_Sierra", "treatment_UV_vs_VIS")), alpha=cutFDR, lfcThreshold=cutLFC)
...
# create the comparison tag
compareTag <- "PA/PA_treatment"
#compareTag <- "Sierra/Sierra_treatment"
#compareTag <- "Olympic/Olympic_treatment"
#compareTag <- "PA_Olympic_Interaction/interaction"
#compareTag <- "PA_Sierra_Interaction/interaction"
#compareTag <- "Olympic_Sierra_Interaction/interaction"

##compareTag <- "Sierra_Olympic"
##compareTag <- "Sierra_Olympic_treatment"
##compareTag <- "Old_New"
##compareTag <- "Old_New_treatment"
...
write.csv(res05_out, file=paste(compareTag, "results.csv", sep="_"), quote = FALSE, row.names = FALSE)
...
# save the plot to a png file
ggsave(paste(compareTag, "vst_dge.png", sep="_"), plot = vst_dge, bg = "white", device = "png", width = 9, height = 8, units = "in")
...
# save the plot to a png file
png(paste(compareTag, "samples_log2fc.png", sep="_"))
...
# save the plot to a png file
png(paste(compareTag, "shrunken_log2fc.png", sep="_"))
