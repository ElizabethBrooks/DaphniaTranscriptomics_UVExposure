...
# save the PCA
pcaData <- plotPCA(vsd, intgroup=c("treatment"), returnData=FALSE)

# customize the PCA
ggplot(pcaData@data, aes(PC1, PC2, color=treatment)) +
  geom_point(size=3) +
...
# store the PCA plot
sample_pca <- ggplot(pcaData@data, aes(PC1, PC2, color=treatment)) +
  geom_point(size=3) +
  xlab(pcaData@labels$x) +
  ylab(pcaData@labels$y) + 
  coord_fixed() + 
...
# treatment UV vs Control indicates that the estimates are of the 
